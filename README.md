...
| 実験用（都度作る） | `main` を継ぐ | 試したい設定。CI が緑になったら `main` にマージ、駄目ならブランチごと削除 |
...
### Studio で物理レイアウトを変えるとコンボが死ぬ

DYA Studio のキーマップタブで物理レイアウト（L Layout / S Layout）を変えると、
キー番号と物理キーの対応がずれてコンボが発動しなくなります。
コンボは `key-positions` 番号基準のためです（flavor・idle の設定は無関係でした）。

直し方: 物理レイアウトを元に戻して保存し直すこと。戻しても直らなければ
キーマップタブのリセット（DT既定に戻る。Studio側キーマップ編集は消える）。
